...
const LatestJobs = () => {
    ...
    return (
        <div className="latestjobs-root" style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px 80px' }}>
            {/* Responsive adjustments only - all original styles/markup untouched */}
            <style>{`
                @media (max-width: 640px) {
                    .latestjobs-root { padding: 28px 16px 56px !important; }
                    .latestjobs-header { align-items: flex-start !important; }
                    .latestjobs-heading { font-size: 28px !important; }
                    .latestjobs-cta { width: 100%; justify-content: center !important; }
                }

                @media (max-width: 420px) {
                    .latestjobs-grid { grid-template-columns: 1fr !important; }
                    .latestjobs-heading { font-size: 24px !important; }
                }
            `}</style>

            <div className="latestjobs-header" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '36px', flexWrap: 'wrap', gap: '16px' }}>
                <div>
                    ...
                    <h2 className="latestjobs-heading" style={{ fontFamily: 'Syne, sans-serif', fontWeight: '800', fontSize: '36px', color: 'var(--text-primary)', lineHeight: '1.2' }}>
                        Latest & Top<br />
                        <span className="gradient-text">Job Openings</span>
                    </h2>
                </div>
                <button className="latestjobs-cta" onClick={() => navigate('/jobs')} style={{
            ...
            {allJobs.length === 0 ? (
                ...
                <div className="latestjobs-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(310px, 1fr))', gap: '16px' }}>
